...
AppName=Space Rangers Universe Redux 1.3.48
AppVersion={#VerDate}
AppVerName=Space Rangers Universe Redux 1.3.48
...
OutputDir=.\Build
DefaultDirName=\
OutputBaseFilename=Universe_Redux_1.3.48
...
Name: DeleteMatrix; Description: {cm:DeleteMatrix}; Types: custom;
Name: InstallModCFG; Description: {cm:InstallModCFG}; Types: custom;
Name: InstallGameSettings; Description: {cm:InstallGameSettings}; Types: custom;
...
Source: GameSettings\newgame.txt; DestDir: {userdocs}\SpaceRangersHD; Flags: ignoreversion; Components: InstallGameSettings;           
Source: RobotsCFG\RobotsCFG.txt; DestDir: {userdocs}\SpaceRangersHD; Flags: ignoreversion;
...
en.DeleteMatrix=Clean the Matrix folder (fix)
en.InstallModCFG=Enable all installed mods in the game
en.InstallGameSettings=Install recommended game settings
// Russian
ru.GameExist=В указанной директории игра не обнаружена, проверьте правильность пути!
ru.GameNotFound=Steam или GOG версия игры не обнаружена, укажите путь вручную (Выберите именно папку с игрой. Папка Mods, например, НЕ подойдёт)
...
ru.DeleteMatrix=Очистить папку Matrix (фикс)
ru.InstallModCFG=Подключить установленные моды (рекомендованные) к игре
ru.InstallGameSettings=Установить рекомендованные настройки партии
...
begin
  ...
  begin
    ...
    RenameFile(ExpandConstant('{app}\Mods\Tweaks\Spanish'), ExpandConstant('{app}\TempStorageFolder\Spanish'));
    ...
    RenameFile(ExpandConstant('{app}\TempStorageFolder\Spanish'), ExpandConstant('{app}\Mods\Tweaks\Spanish'));
  ...
  if WizardIsComponentSelected('DeleteMatrix') then
  begin
    ForceDirectories(ExpandConstant('{app}\TempStorageFolder'));
    RenameFile(ExpandConstant('{app}\Matrix\Map'), ExpandConstant('{app}\TempStorageFolder\Map'));
    DelTree(ExpandConstant('{app}\Matrix\*'), false, true, true);
    ForceDirectories(ExpandConstant('{app}\Mods\Tweaks'));
    RenameFile(ExpandConstant('{app}\TempStorageFolder\Map'), ExpandConstant('{app}\Matrix\Map'));
    RemoveDir(ExpandConstant('{app}\TempStorageFolder'));
  end;
end;
